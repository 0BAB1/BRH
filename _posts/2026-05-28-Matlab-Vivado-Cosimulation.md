...
Now this is the part that confused me the most. Matlab apparently don't really like to works with your simulator because it somehow can't figure out timing (like uou can't enforce the use of timing pragmas inside the VHDL code).

So you end up having to specify, by double clicking the "vivado sim" module (your HDL dut) in MATLAB, go into timing, and specify that 1s in Simulink, s indeed 1s in the DUT. (duh).

Big L for Mathworks on that, that is not only super confusing, not on the **how**, but on the **why** we even have to do that in the first place. And also because this "feature" is paced with "bugs".

It will most likely try to suggest to you that 1s in simulink should be 2e-9 seconds in the DUT. which is not even a single clock cycle. Like, Why ? And if you try to force 1s = 1s, it will likely refuse. *Because why on earth would you want to actually simulate a system lol* ?

Anyway, a setup I found to be working is to keep 1ps of resulotion when inporting the HDL file (otherwise the wrapper willl clash because of their 1ps autogenerated pragma and you'll have to modify the file manually, doable but annoying), then in timing , use 1s = 1000000000ns in HDL.

{: .prompt-info }
> You can find some official "resources" here from mathworks on that matter : https://www.mathworks.com/help/hdlverifier/ug/simulation-timescales.html If you want some actual details on how it works. I personnally just try my my to get to 1s = 1000000000ns equivalent for a "real time" simulation.
